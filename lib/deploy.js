 ...
 * @params {config} 配置文件
 * @params {dockerSure} 是否更新docker 资源
 */
async function deployCommon(config, dockerSure) {
  try {
    ...
    if (stat.isDirectory()) {
      await uploadFile(webDir, distPath + ".zip");

      await unzipFile(webDir, lastPathFromFullPath(distPath));
      if (config.dockerName && dockerSure) {
 ...
 * @param distPath 上传到服务器上的文件或文件夹压缩的zip文件
 */
async function uploadFile(dir, distPath) {
  try {
    const stat = fs.lstatSync(path.resolve(projectDir, distPath));
    const localPath = `${projectDir}/${distPath}`;
    if (stat.isDirectory()) {
      await ssh.putFile(localPath, `${dir}/${distPath}`);
    }
    if (stat.isFile()) {
      const file = lastPathFromFullPath(distPath);
      await ssh.putFile(localPath, `${dir}/${file}`);
    }
    console.log(`（*）上传至目录${underlineLog(dir)}`);
...
async function unzipFile(dir, distPath) {
  try {
    ...
    await runCommand(`unzip -o ${distPath}.zip && rm -f ${distPath}.zip`, dir);
    successLog("  zip包解压成功");
  } catch (err) {
...
async function updateDocker(config) {
  try {
    ...
    // 先删除docker中文件
    await runCommand(
      `docker exec -i ${dockerName} rm -rf ${lastPathFromFullPath(distPath)}`,
      webDir
    );
    ...
    if (stat.isDirectory()) {
      // 上传文件夹到docker;
      await runCommand(
        `docker cp ${webDir} ${dockerName}:${dockerWebDir}`,
        webDir
      );
    }
    if (stat.isFile()) {
      // 上传文件到docker
      const _filePath = `${webDir}/${lastPathFromFullPath(distPath)}`;
      await runCommand(
        `docker cp ${_filePath} ${dockerName}:${dockerWebDir}`,
        webDir
      );
    }
