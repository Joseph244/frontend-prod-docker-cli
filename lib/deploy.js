...
async function updateDocker(config) {
    try {
        console.log(`（7）开始更新docker:${config.dockerName}`);
        await runCommand(`docker cp dist ${config.dockerName}:${config.dockerWebDir}`, config.webDir);
