...
async function deployMuti(projectConfig, deployConfigAll, deployChoices) {
  try {
    ...
    // console.log("所有环境：", deployConfigAll);
...
async function uploadFile(dir, distPath) {
  try {
    ...
    await ssh.putFile(`${projectDir}/${distPath}`, `${dir}/${distPath}`);
    successLog("  zip包上传成功");
  } catch (err) {
...
async function updateDocker(config) {
  try {
    infoLog(`（*）开始更新docker:${config.dockerName}`);
    await runCommand(`cd ${config.webDir}`, config.webDir);
    // 先删除docker中文件
    await runCommand(
      `docker exec -i ${config.dockerName} rm -rf ${config.distPath}`,
      config.webDir
    );
